...
- 62f0ac0: **Types:** Statically resolved return type for `get_node()`/`getNode()`.
- 62f0ac0: **Types:** `ResourceLoader.load()` overload type.
- e5d3b1a: **Fix:** `@ExportObject(Node)` was only working for sub-classes, not `Node` itself.
- e3391d9: **Fix:** Duplicate "Generating" label appeared in the UI during codegen.
- 45deef3: **Types:** `GDictionary` absent values are now correctly typed as returning as `null`, not `undefined`.
- 62f0ac0: **Feature:** Improved type conversion error messages.
- 62f0ac0: **Fix:** `@export_` support for enums as keys of `GDictionary`.
- 264e7b0: **Types/Fix:** Invalid codegen for certain types (function literals in particular)
- 62f0ac0: **Fix:** Background thread script instantiation (for 4.5 editor).
- 62f0ac0: **Fix:** Handle `hint_string` if `@export_var` is a `GArray` with an element type provided via `details.class_`.
- b11cb9c: **Types:** Added `Resource` `duplicate()` return type.
- 62f0ac0: **Feature:** Improved user project type configurability.
...
- 62f0ac0: **Feature:** Ergonomics overhaul. Camel-case, TS types, codegen + more.
...
- 1919a73: **Feature:** Godot String methods now exposed as static methods on `String`.
...
- b3e40c4: **Feature:** Constructor params support and GDScript compatible `new`
...
- a88eb3b: **Types:** Improved GObject types
- 3534db7: **Fix:** Bind script instance during default prop evaluation.
...
- 62f0ac0: **Feature:** Introduced a setting to control whether `.d.ts` for scenes are auto-generated on when scenes are saved in the Editor.
- 66fdfb3: **Feature:** Expose Godot Variant utility method typeof() as `godot_typeof()`/`godotTypeof()`.
- 4a117c2: **Fix:** Threading / Godot address reuse crash.
- 62f0ac0: **Types:** Codegen + type checking for animations.
- 62f0ac0: **Feature:** `CameraFeed` types.
- 24d4f31: **Types/Fix:** Codegen now types more `GArray`/`GDictionary` generic params based on exported variable hint strings.

  Additionally, fixed some codegen for enums when camel-case bindings are enabled.

- 62f0ac0: **Feature:** Support async module loader in quickjs.impl.
- 9bb4c1a: **Feature:** Godot 4.5 support
- 5aedce6: **Types:** "Generate Godot d.ts" in the UI is now "Generate types" and in addition to
  ...
  pollute the project.
- 71047ff: **Fix:** Duplicate PackedByteArray to_array_buffer() registration
- a04eb88: **Fix:** Added missing `PROPERTY_USAGE_SCRIPT_VARIABLE` flag on exported variables.
- 7fb7967: **Types:** `NodePathMap` now permits `undefined`/optional children.
- 62f0ac0: **Feature/Types:** Types + codegen for project input actions.
- 03d2cc4: **Types:** `EditorUndoRedoManager` API types
- ddb38f8: **Types**: Ensure the GAny union type includes null
- 4c52394: **Fix:** Critical bug fixes (for crashes) that may occur due to the
  current isolate not being set.
- 62f0ac0: **Feature:** `GDictionary`/`GArray` recursive handling and `toJSON()`/`toString()`
...
- dab705b: **Fix:** TStringNameCache v8::String reference loss

  https://github.com/godotjs/GodotJS/issues/110

- 254f3a4: **Feature:** Native object now guaranteed bound during JS initializaation.
...
- d3bc8cd: **Fix:** `JSWorker` transfer crash.
- 62f0ac0: **Feature:** Async module loader.
- b6b7972: **Types:** Improved generated typings formatting.
- 62f0ac0: wip: gdextension support
- 62f0ac0: chore: add empty PresetSource constructor for clarity
- 820b9d3: **Fix:** Ensure usercode triggered from `get('prop')`/`set('prop')` does not crash the process, JS runtime errors are caught and logged.
- 62f0ac0: **Fix:** `godot.lib.api` `instanceof` checks against proxied classes.
- 5afe77a: **Fix:** Don't crash when a GodotJS class implementation's parent class does not match the attached node.

  Instead, we log an error and prevent the script from being instantiated.

- 26a2cc1: **Fix:** Enum codegen (and godot.lib.api iteration).
- 62f0ac0: **Performance:** Added LRU support for `StringNameCache`.
- 62f0ac0: **Types**: Refined `SceneTree` `GArray` return types.
- 9d06c8e: **Fix:** Don't leak temporary default properties object
- 62f0ac0: **Feature:** Introduced settings to ensure dependencies are included in exported builds.
...
- 62f0ac0: **Feature:** Godot 4.4 support.
- 62f0ac0: **Fix:** Improved TypeScript class matching regex.
...
- 298d14f: **Fix:** Another missing `Isolate::Scope` that could lead to a runtime crash.
- 62f0ac0: **Fix/Types:** Ensure that codegen quotes property keys when necessary.
- 62f0ac0: **Feature:** Support for importing `.json` files.
- db738ec: **Feature:** New decorator syntax for exporting properties.
...
- 62f0ac0: **Fix:** Always use thread-safe variant pool allocator.
- d4bc651: **Types:** Added types for `Node` direct child APIs:
...
- 62f0ac0: **Feature:** Module resolution now supports modules that utilize `exports` in their `package.json`.
- 62f0ac0: **Fix:** Do not instantiate `GodotJSMonitor` if the JS runtime does not support `Performance`.
- d3e28ea: **Fix/Types:** Don't allow codegen failures to output malformed types.
- 62f0ac0: **Feature:** Added settings to optionally generate `.d.ts` files for Godot scene files in the Editor.
- 62f0ac0: fix: exported properties no longer leak into the base class.
...
- e7a159e: **Types:** GDictionary keys() return type
- b1f7527: **Types:** ResolveNodePathMap utility type.
...
- f06c17a: **Fix:** Variant constructor failure on first bound Variant class.
- 62f0ac0: **Fix:** `@export_dictionary` decorator.
- 71bc6db: **Fix:** Don't crash when script binding fails.
- 62f0ac0: **Fix:** NIL is now permitted as convertible to any other variant types.
- d56dbbc: **Types:** `UserTypeDescriptor` `resource` property is now type-safe and will auto-complete, accepting `Script` resources.
- d107106: **Fix:** Ensure resource types are (re)generated when a scene saves
- a4b4e40: **Types:** Dynamic dispatch methods (e.g.,`call_deferred`) are now typesafe.
...
- 62f0ac0: **Types:** Godot/GDScript best effort typing for nullability of Object types.
...
- 62f0ac0: **Fix:** Support for script instantiation from a ResourceLoader executing on another thread.
- 090a66f: **Fix/Types:** Ensure GDScript singleton class accessors/modifiers are `static`.

  Fixes https://github.com/godotjs/GodotJS/issues/99

- 62f0ac0: **Fix:** Adhere to typical JS semantics and use default arg when undefined is passed (not just when the parameter is omitted).
- 62f0ac0: **Feature:** Added `@export_object` shorthand decorator for exporting objects.
- 62f0ac0: **Feature:** Ensure string representation of Godot objects include the script class name.
- 8e232b7: **Types:** Improved support for statically typed optional nodes.
- 3acb251: **Feature:** Worker postMessage support for structured clone and transfer.
...
- dfb87b3: **Feature:** `GArray` tuple support.

  For example, with the type `GArray<[number, string]>` `.get(0)` will return a `number` and `.get(1)` will return a `string`.

- 62f0ac0: **Types:** Codegen — treat Signal args as output types, not input types.
- cfdeb68: **Types:** `get_tree()`/`getTree()` no longer types as returning `null`.
...
- 62f0ac0: **Fix:** TypeLoader `post_bind` now supports reentrancy.
- 62f0ac0: **Feature/Types:** Codegen/types for resources in your project i.e., `ResourceLoader.load("res://whatever")` is now strongly typed depending on the file path.
- 62f0ac0: **Feature:** API to access `GDictionary`/`GArray` like JS objects/arrays (`.proxy()`) and
  improved type definitions for `GDictionary` and `GArray`.
- 77e3dba: **Fix:** Make Variant constants (e.g., `Vector3.ZERO`) Readonly<> so they can't (easily) be accidentally mutated.
