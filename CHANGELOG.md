...
- 9b917c2: **Types:** Statically resolved return type for `get_node()`/`getNode()`.
- 9b917c2: **Types:** `ResourceLoader.load()` overload type.
- 8664bf2: **Fix:** `@ExportObject(Node)` was only working for sub-classes, not `Node` itself.
- fd177b9: **Fix:** Duplicate "Generating" label appeared in the UI during codegen.
- aebbea4: **Types:** `GDictionary` absent values are now correctly typed as returning as `null`, not `undefined`.
- 9b917c2: **Feature:** Improved type conversion error messages.
- 9b917c2: **Fix:** `@export_` support for enums as keys of `GDictionary`.
- 047f35a: **Types/Fix:** Invalid codegen for certain types (function literals in particular)
- 9b917c2: **Fix:** Background thread script instantiation (for 4.5 editor).
- 9b917c2: **Fix:** Handle `hint_string` if `@export_var` is a `GArray` with an element type provided via `details.class_`.
- e427909: **Types:** Added `Resource` `duplicate()` return type.
- 9b917c2: **Feature:** Improved user project type configurability.
...
- 9b917c2: **Feature:** Ergonomics overhaul. Camel-case, TS types, codegen + more.
...
- 3bb0f5e: **Feature:** Godot String methods now exposed as static methods on `String`.
...
- 2461614: **Feature:** Constructor params support and GDScript compatible `new`
...
- 453da8e: **Types:** Improved GObject types
- 3369ccd: **Fix:** Bind script instance during default prop evaluation.
...
- 9b917c2: **Feature:** Introduced a setting to control whether `.d.ts` for scenes are auto-generated on when scenes are saved in the Editor.
- a14dfe0: **Feature:** Expose Godot Variant utility method typeof() as `godot_typeof()`/`godotTypeof()`.
- 4ac8479: **Fix:** Threading / Godot address reuse crash.
- 9b917c2: **Types:** Codegen + type checking for animations.
- 9b917c2: **Feature:** `CameraFeed` types.
- 1ed2957: **Types/Fix:** Codegen now types more `GArray`/`GDictionary` generic params based on exported variable hint strings.

  Additionally, fixed some codegen for enums when camel-case bindings are enabled.

- 9b917c2: **Feature:** Support async module loader in quickjs.impl.
- e0641c8: **Feature:** Godot 4.5 support
- 18a6ff5: **Types:** "Generate Godot d.ts" in the UI is now "Generate types" and in addition to
  ...
  pollute the project.
- e35d35e: **Fix:** Duplicate PackedByteArray to_array_buffer() registration
- 8394f98: **Fix:** Added missing `PROPERTY_USAGE_SCRIPT_VARIABLE` flag on exported variables.
- e7a6713: **Types:** `NodePathMap` now permits `undefined`/optional children.
- 9b917c2: **Feature/Types:** Types + codegen for project input actions.
- d933753: **Types:** `EditorUndoRedoManager` API types
- 39a01db: **Types**: Ensure the GAny union type includes null
- 05647ac: **Fix:** Critical bug fixes (for crashes) that may occur due to the
  current isolate not being set.
- 9b917c2: **Feature:** `GDictionary`/`GArray` recursive handling and `toJSON()`/`toString()`
...
- c01c0d4: **Fix:** TStringNameCache v8::String reference loss

  https://github.com/godotjs/GodotJS/issues/110

- d3b1ca7: **Feature:** Native object now guaranteed bound during JS initializaation.
...
- abd0137: **Fix:** `JSWorker` transfer crash.
- 9b917c2: **Feature:** Async module loader.
- 9ea9ea1: **Types:** Improved generated typings formatting.
- e03e7c4: **Fix:** Ensure usercode triggered from `get('prop')`/`set('prop')` does not crash the process, JS runtime errors are caught and logged.
- 9b917c2: **Fix:** `godot.lib.api` `instanceof` checks against proxied classes.
- 762de54: **Fix:** Don't crash when a GodotJS class implementation's parent class does not match the attached node.

  Instead, we log an error and prevent the script from being instantiated.

- 6770b97: **Fix:** Enum codegen (and godot.lib.api iteration).
- 9b917c2: **Performance:** Added LRU support for `StringNameCache`.
- 9b917c2: **Types**: Refined `SceneTree` `GArray` return types.
- 5e0104e: **Fix:** Don't leak temporary default properties object
- 9b917c2: **Feature:** Introduced settings to ensure dependencies are included in exported builds.
...
- 9b917c2: **Feature:** Godot 4.4 support.
- 9b917c2: **Fix:** Improved TypeScript class matching regex.
...
- 9c69889: **Fix:** Another missing `Isolate::Scope` that could lead to a runtime crash.
- 9b917c2: **Fix/Types:** Ensure that codegen quotes property keys when necessary.
- 9b917c2: **Feature:** Support for importing `.json` files.
- 36c2a9c: **Feature:** New decorator syntax for exporting properties.
...
- 9b917c2: **Fix:** Always use thread-safe variant pool allocator.
- a359d04: **Types:** Added types for `Node` direct child APIs:
...
- 9b917c2: **Feature:** Module resolution now supports modules that utilize `exports` in their `package.json`.
- 9b917c2: **Fix:** Do not instantiate `GodotJSMonitor` if the JS runtime does not support `Performance`.
- 7372ba3: **Fix/Types:** Don't allow codegen failures to output malformed types.
- 9b917c2: **Feature:** Added settings to optionally generate `.d.ts` files for Godot scene files in the Editor.
- 9b917c2: fix: exported properties no longer leak into the base class.
...
- 3bd9856: **Types:** GDictionary keys() return type
- 5ebf454: **Types:** ResolveNodePathMap utility type.
...
- 143980a: **Fix:** Variant constructor failure on first bound Variant class.
- 9b917c2: **Fix:** `@export_dictionary` decorator.
- 95cf6b6: **Fix:** Don't crash when script binding fails.
- 9b917c2: **Fix:** NIL is now permitted as convertible to any other variant types.
- 0c57517: **Types:** `UserTypeDescriptor` `resource` property is now type-safe and will auto-complete, accepting `Script` resources.
- 0f173d3: **Fix:** Ensure resource types are (re)generated when a scene saves
- f9f53dd: **Types:** Dynamic dispatch methods (e.g.,`call_deferred`) are now typesafe.
...
- 9b917c2: **Types:** Godot/GDScript best effort typing for nullability of Object types.
...
- 9b917c2: **Fix:** Support for script instantiation from a ResourceLoader executing on another thread.
- 66e85b2: **Fix/Types:** Ensure GDScript singleton class accessors/modifiers are `static`.

  Fixes https://github.com/godotjs/GodotJS/issues/99

- 9b917c2: **Fix:** Adhere to typical JS semantics and use default arg when undefined is passed (not just when the parameter is omitted).
- 9b917c2: **Feature:** Added `@export_object` shorthand decorator for exporting objects.
- 9b917c2: **Feature:** Ensure string representation of Godot objects include the script class name.
- 957d5ca: **Types:** Improved support for statically typed optional nodes.
- 4ae63a9: **Feature:** Worker postMessage support for structured clone and transfer.
...
- ad96db5: **Feature:** `GArray` tuple support.

  For example, with the type `GArray<[number, string]>` `.get(0)` will return a `number` and `.get(1)` will return a `string`.

- 9b917c2: **Types:** Codegen — treat Signal args as output types, not input types.
- 7626e73: **Types:** `get_tree()`/`getTree()` no longer types as returning `null`.
...
- 9b917c2: **Fix:** TypeLoader `post_bind` now supports reentrancy.
- 9b917c2: **Feature/Types:** Codegen/types for resources in your project i.e., `ResourceLoader.load("res://whatever")` is now strongly typed depending on the file path.
- 9b917c2: **Feature:** API to access `GDictionary`/`GArray` like JS objects/arrays (`.proxy()`) and
  improved type definitions for `GDictionary` and `GArray`.
- 80db70d: **Fix:** Make Variant constants (e.g., `Vector3.ZERO`) Readonly<> so they can't (easily) be accidentally mutated.
