    ...
    private:
        ...
        static void constructor(const v8::FunctionCallbackInfo<v8::Value>& info)
        {
            ...
            if (new_target->Get(context, jsb_symbol(environment, CrossBind)).ToLocal(&cross_bind_sym) && cross_bind_sym->IsString())
            {
                ...
                std::vector<v8::Local<v8::Value>> constructor_args;
                constructor_args.reserve(arg_count);

                for (int i = 0; i < arg_count; ++i) {
                    constructor_args.push_back(info[i]);
                }

                ScriptClassInfo::instantiate(environment, script_module_id, self, constructor_args);
                return;
            }
